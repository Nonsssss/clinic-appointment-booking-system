...
# FIX: removed prefix="/auth" here — main.py already mounts this router with prefix="/auth"
# Having it in both places caused routes to be /auth/auth/login
...
async def signup(payload: SignupSchema, db: Session = Depends(get_db)):
    try:
        ...
        new_user = User(fullname=fullname, email=email, password=hashed_pass)
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
